...
{
    public class Episode12Regen : Game
    {
        ...
        protected override void Initialize()
        {
            ...
            for (int i = 0; i < NO_OF_PARTICLES; i++)
            {
                ...
                _particles[i].position = new Vector2(_width / 2, _height / 4);
                ...
                _particles[i].gravity.Y = 1f;
                _particles[i].friction = 0.95f;
                _particles[i].Scale /= 8;
            }
        }
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            for (int i = 0; i < NO_OF_PARTICLES; i++)
            {
                if (_particles[i].position.Y - _particles[i].Radius > _width)
                {
                    _particles[i].position.X = _width / 2;
                    _particles[i].position.Y = _height / 4;
                    _particles[i].velocity.SetLength(CommonFunctions.RandomRange(4, 8));
                    _particles[i].velocity.SetAngle(CommonFunctions.RandomRange(0, MathF.PI * 2));
                }
                _particles[i].Update();
